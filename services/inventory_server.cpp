...
class InventoryServiceImpl final : public SagaParticipant::Service {
...
public:
    Status ExecuteAction(ServerContext* context, const TxRequest* request, TxResponse* reply) override {
        ...
        if (request->payload().find("fail_inventory") != std::string::npos) {
            std::cout << "[Inventory] FAILED deduction for TxID: " << request->tx_id() << std::endl;
            reply->set_success(false);
            reply->set_error_message("Item out of stock");
            return Status::OK;
        }

        std::cout << "[Inventory] Deducted items for TxID: " << request->tx_id() << std::endl;
    ...
    Status CompensateAction(ServerContext* context, const TxRequest* request, TxResponse* reply) override {
        ...
        if (processed_tx.find(request->tx_id()) != processed_tx.end()) {
            std::cout << "[Inventory] ROLLED BACK items for TxID: " << request->tx_id() << std::endl;
